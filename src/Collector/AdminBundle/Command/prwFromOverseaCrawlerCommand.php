...
class prwFromOverseaCrawlerCommand extends ContainerAwareCommand
{

    protected function configure()
    {
        $this
            ->setName('collector:admin:prwfromoversea_crawler')
            ->setDescription('get prwfromoversea press data')
            ->addArgument(
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        elseif ($range == 'all')
            $pageNum = 100;
        $pressSource = 'http://prw.kyodonews.jp/opn/category/release/kaigaikara/page/1/';
        for ($page = 1; $page <= $pageNum; $page++) {
            $pressList = "http://prw.kyodonews.jp/opn/category/release/kaigaikara/page/" . $page;
