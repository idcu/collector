...
class prwSpecicalCrawlerCommand extends ContainerAwareCommand
{

    protected function configure()
    {
        $this
            ->setName('collector:admin:prwspecical_crawler')
            ->setDescription('get prwspecical press data')
            ->addArgument(
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        elseif ($range == 'all')
            $pageNum = 100;
        $pressSource = 'http://prw.kyodonews.jp/opn/category/release/tokuhain/page/1/';
        for ($page = 1; $page <= $pageNum; $page++) {
            $pressList = "http://prw.kyodonews.jp/opn/category/release/tokuhain/page/" . $page;
