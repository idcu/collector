...
class prwJpCrawlerCommand extends ContainerAwareCommand
{

    protected function configure()
    {
        $this
            ->setName('collector:admin:prwjp_crawler')
            ->setDescription('get prwjp press data')
            ->addArgument(
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        elseif ($range == 'all')
            $pageNum = 100;
        $pressSource = 'http://prw.kyodonews.jp/opn/news-release/';
        for ($page = 1; $page <= $pageNum; $page++) {
            $pressList = "http://prw.kyodonews.jp/opn/news-release/" . $page;
            ...
            foreach ($pressUrls as $pressUrl) {
                ...
                $press['company_name'] = $crawler->filter('.releaseText>.rel_hakko')->text();
                ...
                $imageFiles = $crawler->filter('.slide>ul>li>a')->each(function (Crawler $node, $i) {
